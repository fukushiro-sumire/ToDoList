class TasksController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @task.save
        format.html { redirect_to @task, notice: 'タスク登録しました。' }
  ...
  def update
    respond_to do |format|
      if @task.update(task_params)
        format.html { redirect_to @task, notice: 'タスク変更しました。' }
  ...
  def destroy
    @task.destroy
    respond_to do |format|
      format.html { redirect_to tasks_url, notice: 'タスク削除しました' }
 ...
 def search  
  ...
  private
    ...
    def task_params
      params.require(:task).permit(:due_date, :category_id, :name, :dane, :user_id)
    end
end
